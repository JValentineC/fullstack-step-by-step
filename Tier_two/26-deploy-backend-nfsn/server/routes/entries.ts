...
router.get('/', async (req, res) => {
  ...
  const dataSql = `SELECT * FROM Entry${whereClause} ORDER BY \`${sort}\` ${order} LIMIT ? OFFSET ?`
  const countSql = `SELECT COUNT(*) AS total FROM Entry${whereClause}`
...
router.post('/', requireAuth, async (req, res) => {
  ...
  const [rows] = await pool.execute<EntryRow[]>(
    'SELECT * FROM Entry WHERE id = ?', [result.insertId]
  )
